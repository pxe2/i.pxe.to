...
curl -O http://a737b1065522.pouliot.net/coreos/cloud-config.yml/coreos-stable-amd64.cloud-config.yml
sed -i "/hostname: coreos/c\hostname: $NEWHOSTNAME" ./coreos-stable-amd64.cloud-config.yml


#sed -i "/ discovery: \"https://discovery.etcd.io/<token>\"/c\ $DISCOVERY" ./coreos-stable-amd64.cloud-config.yml
...
then
 ...
 # Zero MBR
  ...
  sudo coreos-install -d /dev/vda -C stable -c coreos-stable-amd64.cloud-config.yml
fi
...
then
  ...
  sudo coreos-install -d /dev/sda -C stable -c coreos-stable-amd64.cloud-config.yml
fi
